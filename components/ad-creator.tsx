...
export default function AdCreator() {
  ...
  const handleGenerate = async () => {
    ...
    if (isAnonymous) {
      // If trying to use non-square formats, show paywall
      if (!isOnlySquareFormat) {
        setShowPaywall(true)
        return
      }
  ...
  const getAvailableAspectRatios = () => {
    // All users with Pro or Business subscription get all aspect ratios
    if (
      subscription.status === "pro" ||
      subscription.status === "business" ||
      user?.email?.toLowerCase() === "cameronnpost@outlook.com"
    ...
    // Anonymous users only get square format
    if (isAnonymous) {
      return [{ id: "1:1", label: "1:1 (Square) - 3 free" }]
    }

    // Free users get limited aspect ratios
    return [
  ...
  return (
    <>
            ...
            <div className="space-y-6">
                    ...
                    <Alert className="bg-primary/10 border-primary/20">
                      ...
                      <AlertDescription className="flex flex-col gap-1">
                        ...
                        <span className="text-sm">
                          We've analyzed <span className="font-medium">{brandAnalysis.title || brandAnalysis.url}</span>{" "}
                          and will use this information to tailor your ads.
                        </span>
              ...
              <Tabs defaultValue="prompt" className="w-full">
                <TabsList className="grid w-full grid-cols-2 bg-background">
                  <TabsTrigger value="prompt" className="data-[state=active]:bg-primary data-[state=active]:text-white">
                    Text Prompt Only
                  </TabsTrigger>
              ...
              <div className="space-y-3">
                ...
                {isAnonymous ? (
                  <p className="text-xs text-muted-foreground">
                    Guest users can create up to 3 square format (1:1) ads for free. Sign up for more options.
                  </p>
                ) : subscription.status === "free" ? (
                  <p className="text-xs text-muted-foreground">
                    Free plan: 3 square format (1:1) ads available. Upgrade to Pro or Business for unlimited ads in all
                    formats.
                  </p>
                ) : null}
              </div>
                ...
                >
                  {isGenerating ? (
                    ...
                    <>
                      ...
                      Generate {brandAnalysis ? "Brand-Tailored" : ""} Ads
      ...
      <PaywallModal
        ...
        user={user}
        subscription={subscription}
        squareFormatOnly={selectedAspectRatios.length === 1 && selectedAspectRatios[0] === "1:1"}
      />
    </>
  )
}
